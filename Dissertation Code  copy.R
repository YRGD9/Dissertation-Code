...
indep_vars_tp_types <- peace[,c("local_tp", "west_tp", "int_tp", "Stage_comp", "Stage_ren", "agtp_inter")] 
...
indep_vars_tp_types_lm <- peace[,c("local_tp", "west_tp", "int_tp", "Stage_comp", "Stage_ren", "agtp_inter")] 

model_types_lm <- lm(dep_var_ended_types_lm ~ local_tp + west_tp + int_tp + agtp_inter + Stage_comp + Stage_ren, data = peace, family = binomial)
...
indep_vars_tp_ref <- peace[,c("local_tp", "west_tp", "int_tp", "GDis", "GMig", "GRa", "GRe", "GOth", "GSoc", "GeMe", "GeFa")]

#Model: 
model_ref <- glm(dep_var_ended_ref ~ local_tp + west_tp + int_tp + GDis + GMig + GRa + GRe + GOth + GSoc + GeMe + GeFa, data = peace, family = binomial)
summary(model_ref) 

stargazer(model_ref, type = "text")

# local_tp                     0.189       (0.163)          
# west_tp                     -0.521*      (0.271)          
# int_tp                       0.417**     (0.195) 
# Constant                    -1.276***    (0.123) 
# Observations                  942            
# Log Likelihood               -511.742         
# Akaike Inf. Crit.           1,047.485    
...
hoslem.test(observed_outcomes_ref, predicted_probs_ref) # p-value = 0.996 -- Good fit as over 0.05 significant rate
...
# 0.586 -->  model has only slightly better than chance level predictive ability
...
# west_tp:  95% confidence interval:  0.5937392 0.3492910 1.0092620
#The 95% CI excludes 1, indicating that the effect of west_tp is statistically significant.
...
indep_vars_tp_ref_lm <- peace[,c("local_tp", "west_tp", "int_tp", "GDis", "GMig", "GRa", "GRe", "GOth", "GSoc", "GeMe", "GeFa",)]

model_ref_lm <- lm(dep_var_ended_ref_lm ~ local_tp + west_tp + int_tp + GDis + GMig + GRa + GRe + GOt + GSoc + GeMe + GeFa, data = peace, family = binomial)
...
indep_vars_tp_all <- peace[,c("local_tp", "west_tp", "int_tp", "GDP", "GFS", "GDis", "GMig", "GRa", "GRe", "GOth", "GSoc", "GeMe", "GeFa", "agtp_inter", "Stage_comp", "Stage_ren")]

#Model: 
model_all <- glm(dep_var_ended_all ~ local_tp + west_tp + int_tp + GDP + GFS + GDis + GMig + GRa + GRe + GOth + GSoc + GeMe + GeFa + agtp_inter + Stage_comp + Stage_ren, data = peace, family = binomial)
summary(model_all)

stargazer(model_all, type = "text")

# local_tp                     0.345*       (0.205)          
# west_tp                     -1.846***     (0.343)          
# int_tp                       0.473**      (0.238) 
# Constant                    -3.124***     (0.221)  
# Observations                  942            
# Log Likelihood               -365.599            
# Akaike Inf. Crit.             765.197    
...
# Perform the chi-squared test
chisq.test(observed_predicted_all)    # p-value = 0.001908 --> less than 0.05
# difference between the observed and predicted values is statistically significant and the model is a good fit for the data
...
hoslem.test(observed_outcomes_all, predicted_probs_all) # p-value = 0.5314 --  Fit as under 0.05 significant rate
...
# 0.859 -->  model has only slightly better than chance level predictive ability
...
# west_tp:  95% confidence interval:   0.15779314 0.08062170  0.30883342
#The 95% CI excludes 1, indicating that the effect of west_tp is statistically significant.
...
model_all_lm <- lm(dep_var_ended_all_lm ~ local_tp + west_tp + int_tp + Dat_year + GDP + GFS + GDis + GMig + GRa + GRe + GOth + GSoc + GeMe + GeFa + agtp_inter + Stage_comp + Stage_ren, data = peace, family = binomial)
